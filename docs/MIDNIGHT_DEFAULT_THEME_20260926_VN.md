...
**Chưa có log UART ổn định hoặc ảnh chụp thực tế sau flash để chứng minh migration NVS/hiển thị trên ST7789**: COM3 tiếp tục gặp `WriteFile failed / Access denied` và có lúc mất cổng ngay sau esptool reset. Board trước đó đã từng có `reset=Brownout, safe_mode=1`. Không gỡ chế độ bảo vệ Brownout hay tự xóa Safe Mode. Trước khi thử nghiệm trực quan, cần ổn định nguồn USB/cáp/CH340 và kết nối lại, sau đó gửi `diag theme status` để xác nhận `id=5 name=VQEAF Midnight modern_font=1`. Nếu vẫn ở Recovery, kiểm tra nguồn rồi chọn Start normal mode. Chụp màn Home/Apps/Settings sau khi vào Normal Mode và đo tốc độ cuộn nếu muốn gate đầy đủ trước khi merge PR #10.
