...
#ifdef VQEAF_INPUT_FAKE_CLOCK
extern uint32_t vqeafFakeMillis;
extern int vqeafFakePinState[64];
inline uint32_t millis(){return vqeafFakeMillis;} inline void delay(uint32_t){} inline void pinMode(int,int){}
inline int digitalRead(int pin){return pin>=0&&pin<64?vqeafFakePinState[pin]:HIGH;}
#else
