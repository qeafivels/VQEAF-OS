...
class WiFiClass { public:
 ...
 // Explicitly opt-in for status-bar framebuffer regression. All legacy host
 // suites keep their original disconnected WiFi mock by default.
 int mockStatus=0;
 int32_t mockRssi=-50;
 ...
 int scanNetworks(bool=false,bool=false){return 0;} int scanComplete(){return 0;} String SSID(int=0)const{return String("ssid");} int32_t RSSI(int=0)const{
   ...
   return -50;
#endif
 } wifi_auth_mode_t encryptionType(int)const{return WIFI_AUTH_OPEN;} void scanDelete(){}
