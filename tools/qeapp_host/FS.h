...
public:
  ...
  size_t write(const uint8_t *b,size_t n){if(!h||!h->fp)return 0;size_t r=fwrite(b,1,n,h->fp);if(ftell(h->fp)>=0)h->len=ftell(h->fp);return r;}
...
public:
  ...
  void failNextRenameFrom(const String &path){failRenameOnce=path.c_str();}
  ...
  bool rename(const String &src,const String &dst){
    if(!failRenameOnce.empty() && failRenameOnce==src.c_str()){
      failRenameOnce.clear();return false;
    }
